...
run: async (client, message, args, embed, prefix) => {
    ...
    message.lineReply(`${uye}, ${message.author} senin ses kanalına girmek için izin istiyor! Onaylıyor musun?`).then(async msj => {
      await msj.react('✅');
      msj.awaitReactions(reactionFilter, {max: 1, time: 15000, error: ['time']}).then(c => {
	let cevap = c.first();
	if (cevap) {
	  message.member.voice.setChannel(uye.voice.channelID);
    msj.edit(`${message.author}, ${uye} kişisinin kanalına taşındı!`)
