...
message.react(green)
message.lineReply(`${user} kişisine başarıyla <@&${conf.musician}> rolü verildi!`).then(x=>x.delete({timeout:5000}))
await user.roles.add(conf.musician)
} else {
message.react(green)
message.lineReply(`${user} kişisinden başarıyla <@&${conf.musician}> rolü alındı!`).then(x=>x.delete({timeout:5000}))
await user.roles.remove(conf.musician)
}}
};
