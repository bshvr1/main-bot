...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.channel.send(`Hata yaptın!\`${prefix}kadın <@Aias/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
...
${uye.toString()} üyesi, <@${message.author.id}> tarafından, ${conf.kizRolleri.length > 1 ? conf.kizRolleri.slice(0, -1).map(x => `<@&${x}>`).join(", ") + " ve " + conf.kizRolleri.map(x => `<@&${x}>`).slice(-1) : conf.kizRolleri.map(x => `<@&${x}>`).join("")} rolleri verilerek kayıt edildi! 
